                             ...
                             int32_t zp, float scale) 
{
    ...
    for (int a = 0; a < 3; a++)
    {
        for (int i = 0; i < grid_h; i++)
        {
            ...
            {
                float obj_conf = sigmoid(deqnt_affine_to_f32(input[tensor_len * (i * grid_w + j) + 4 + PROP_BOX_SIZE*a], zp, scale));
                if (obj_conf < threshold) continue;
                float cls_conf = sigmoid(deqnt_affine_to_f32(input[tensor_len * (i * grid_w + j) + 15 + PROP_BOX_SIZE*a], zp, scale));
                if (cls_conf >= threshold)
                {
                    int offset = tensor_len * (i * grid_w + j) + PROP_BOX_SIZE*a;
                    int8_t *in_ptr = input + offset;

                    int offset1 = tensor_len * (i * grid_w + j) + 5 + PROP_BOX_SIZE*a;
                    ...
                    // landmark
                    for (int k = 0; k < 5; ++k) {
                        int8_t landmx  = in_ptr1[(2*k)];
                        int8_t landmy  = in_ptr1[(2*k+1)];
