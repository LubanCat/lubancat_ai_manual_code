...
int main(int argc, char **argv)
{
    ...
    ret = init_yolo26_obb_model(model_path, &rknn_app_ctx);
    if (ret != 0)
    {
        printf("init_yolo26_obb_model fail! ret=%d model_path=%s\n", ret, model_path);
        goto out;
    }
    ...
    ret = inference_yolo26_obb_model(&rknn_app_ctx, &src_image, &od_results);
    if (ret != 0)
    {
        printf("inference_yolo26_obb_model fail! ret=%d\n", ret);
        goto out;
    }
...
out:
    ...
    ret = release_yolo26_obb_model(&rknn_app_ctx);
    if (ret != 0)
    {
