    ...
    for (int h = 0; h < grid_h; h++) {
        for (int w = 0; w < grid_w; w++) {
            for (int a = 0; a < OBJ_CLASS_NUM; a++) {
                ...
                if(box_conf_f32 >= thres_i8) { //[1,tensor_len,grid_h,grid_w]
                    ...
                    xywh_add[0] = loc[0] + loc[2];
                    xywh_add[1] = loc[1] + loc[3];
                    xywh_sub[0] = (loc[2] - loc[0]) / 2;
                    xywh_sub[1] = (loc[3] - loc[1]) / 2;
                    ...
                    boxScores.push_back(deqnt_affine_to_f32(box_conf_f32, zp, scale));
                    classId.push_back(a);
                    validCount++;
                }
            }
...
static int process_fp32(float *input, float *angle_feature, int grid_h, int grid_w, int stride,
    ...
    for (int h = 0; h < grid_h; h++) {
        for (int w = 0; w < grid_w; w++) {
            for (int a = 0; a < OBJ_CLASS_NUM; a++) {
                float box_conf_f32 = input[(input_loc_len + a) * grid_w * grid_h + h * grid_w + w ]; //[1,tensor_len,grid_h,grid_w]
